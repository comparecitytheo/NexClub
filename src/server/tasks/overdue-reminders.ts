...
export type ReminderResult = {
  ...
  /** Tasks due later today that were reminded in the second pass. */
  remindedDueToday: number;
  dryRun: boolean;
};
...
export async function sendOverdueTaskReminders(
  ...
  if (dryRun) {
    return {
      ...
      // The due-today pass runs after this early return, so a dry run reports
      // nothing for it rather than guessing a count it never computed.
      remindedDueToday: 0,
      dryRun: true,
    };
  }
  ...
  const dueTodayTasks = await prisma.task.findMany({
    // assigneeId is NOT NULL in the schema — every task has an assignee — so
